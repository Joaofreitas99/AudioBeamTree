...
int main(int argc, char **argv){
	...
	WavePtr = 0; 
	//printf("songName = %s\n", songName);
	waveLoad("carrier.wav");
	short *WavePtrC = WavePtr; 
	WavePtr = 0; 
	//waveLoad("multiToneOctave-6.wav");
	waveLoad("everybodyschanging192.wav");
	...
	double complex carrierBuffer[SIZE_OF_BUFFER] = { 0 };	// Empty circular bufferr
	...
	double posProsCarrier[SIZE_OF_BUFFER] = { 0 };
	double anteriorCarrier[SIZE_OF_BUFFER] = { 0 };
	double auxCarrier[SIZE_OF_BUFFER*2] = { 0 };
	double anteriorPosCarrier[SIZE_OF_BUFFER] = { 0 };
	...
	while(waveIndex < WaveSize){
		...
		if(buf_len != SIZE_OF_BUFFER){ // Write
			...
			carrierBuffer[writeIndex++] = (double complex) WavePtrC[waveIndex++]/norm;
			...
			buf_len++;
				...
				for(int i=0; i<SIZE_OF_BUFFER; i++){
						...
						/// Carrier
						auxCarrier[i]=anteriorCarrier[i]* window[i];
				}
				...
				for(int i=0; i<SIZE_OF_BUFFER; i++){
						...
						/// Carrier
						anteriorCarrier[i] = carrierBuffer[i];
						auxCarrier[i+SIZE_OF_BUFFER]= carrierBuffer[i] * window[i+SIZE_OF_BUFFER];
				}
				
				for(int i=0; i<SIZE_OF_BUFFER; i++){
					posPros[i] = ((aux[i]+anteriorPos[i])/3) * (3*(auxCarrier[i]+anteriorPosCarrier[i]));
				}
				
				for(int i=0; i<SIZE_OF_BUFFER; i++){
					anteriorPos[i]=aux[i+SIZE_OF_BUFFER];
					anteriorPosCarrier[i]=auxCarrier[i+SIZE_OF_BUFFER];
					//printf("posPros[%d] = %lf \n", i, posPros[i]);
	...
	waveToFile( &mySound, "testeDSBkeaneNovoAmp.wav");
    waveDestroy( &mySound );  
